...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 30, 2023 at 11:13 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `HeidiSQL_temproutine_1` (IN `p_user_id` INT, INOUT `p_project_list` VARCHAR(2000))   BEGIN
    ...
    WHILE i <= LENGTH(inputString) - LENGTH(REPLACE(inputString, ',', '')) + 1 DO
        ...
        SET outputString = (SELECT CONCAT(project_id, '#', project_name, '#', project_description, '#', project_added_date) FROM tbl_project WHERE user_id = v_userid LIMIT 1);
