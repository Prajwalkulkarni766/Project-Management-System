...
function create_profile_div(is_old, user_id, user_name, user_contact_no, user_email_add, user_gender, user_role, user_password, user_disabled) {
    ...
    if (is_old == 1) {
        ...
        <input type="checkbox" id="existing_user_disabled_' + user_id + '" class="w3-check w3-xlarge" ';
        user_disabled == 1 ? profile += 'title="Disabled User" onchange="disable_or_enable_user(' + user_id + ')" checked' : profile += 'title="Enabled User" onchange="disable_or_enable_user(' + user_id + ')" \
         > \
        <button onclick="save_changes(' + user_id + ')" class="w3-button w3-margin-left w3-xlarge" title="Save Changes"><i class="fa fa-floppy-o" aria-hidden="true"></i></button> \
        </div>';
    }
    profile += '</div> \
    </div> \
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response > 0) {
                ...
                document.getElementById("new-alert-msg").innerHTML = "Please enter unique email address";
                document.getElementById("new-alert").style.display = "block";
            }
        },
    });
