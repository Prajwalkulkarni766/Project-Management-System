...
if (isset($_POST['log_in'])) {
	...
	if ($num) {
		while ($row = mysqli_fetch_array($result)) {
			...
			$_SESSION['privilege_for_chat'] = $row['privilege_for_chat'];
			...
			$get_chatroom_id = mysqli_query($con, "SELECT chatroom_id FROM tbl_chatroom WHERE chatroom_created_by = $superadminid[user_id]");
			$RE = mysqli_fetch_assoc($get_chatroom_id);
			if (isset($RE)) {
				$_SESSION['chatroom_id'] = $RE['chatroom_id'];
			} else {
...
if (isset($_POST['sign_up'])) {
	...
	mysqli_query($con, "CALL project_management_system_sp_for_user(@user_id,'$_POST[user_name]','$_POST[user_contact_no]',$_POST[user_gender_id],1,'$_POST[user_email_add]','$_POST[user_password]',0,0,CURRENT_TIMESTAMP,1)");
	...
	if ($row['user_id'] > 0) {
		...
		if ($num) {
			while ($r = mysqli_fetch_array($result)) {
				...
				$_SESSION['privilege_for_chat'] = $r['privilege_for_chat'];
